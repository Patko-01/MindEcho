            ...
            <form method="POST" action="{{ route('dashboard.newEntry') }}">
                ...
                <div class="input-group rounded-5 border px-3 py-2 flex-column">
                    ...
                    <div class="ms-1 d-flex">
                        <input type="hidden" name="tag" class="form-control form-control-sm w-auto" id="selectedTagInput" value="{{ request('tag', 'Thoughts') }}">
                        <button type="button" class="btn btn-sm text-secondary" id="tagButton">
                            #{{ request('tag', 'Thoughts') }}
                        </button>
        ...
        <div class="dashboard-container-part float-end">
            ...
            @foreach($data as $tag => $entries)
                <div class="mb-3">
                    ...
                    <div id="{{ $tag }}"
                         class="collapse collapse-content fade-smooth mt-2 {{ request('tag') === $tag ? 'show' : '' }}">
                            ...
                            <form method="POST" action="{{ route('dashboard.destroy') }}">
                                ...
                                <div class="item-box mb-2 d-flex gap-3 align-items-center">
                                    <input name="entry_id" value="{{ $entry->id }}"
                                           class="form-check-input submit-on-check" type="checkbox"
                                    ...
                                    <div class="flex-grow-1">
                                        ...
                                        <span class="d-block">
                                            ...
                                            <span
                                                class="item-date d-block">{{ $entry->created_at->format('d.m.Y H:i') }}</span>
                                        </span>
                                        </button>
                                    </div>
                                </div>
